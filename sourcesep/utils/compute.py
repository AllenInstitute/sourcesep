...
def perm_avgabscorr(X, Y, dim=1):
    ...
    for i, perm in enumerate(perm_idx):
        corrs[i] = np.nanmean(
            np.abs([np.corrcoef(X[:, perm[j]], Y[:, j])[0, 1] for j in range(Y.shape[1])]))
...
def test_perm_avgabscorr():
    ...
    assert tuple(perms[np.argmax(z)]) == (
        1, 2, 0), 'permuted correlations failed test'
    return
...
def welch_psd(signal, n_per_segment=100, n_overlap=None, sampling_freq=20, verbose=False):
    ...
    psd_welch = torch.mean(torch.abs(fft_segments)**2,
                           dim=0) / (torch.sum(windows**2))
