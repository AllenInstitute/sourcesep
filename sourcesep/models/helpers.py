...
class H5Dataset(Dataset):
    ...
    def _proc(self, x, idx):
        return np.swapaxes(x[idx:idx+self.n_timesamples].reshape(self.n_timesamples, -1), axis1=0, axis2=1).astype(np.float32)
...
class H5DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str):
        if stage == "fit":
            self.train_dataset = H5Dataset(h5_filename=self.train_h5_filename,
                                           n_timesamples=self.n_timesamples)
            self.val_dataset = H5Dataset(h5_filename=self.val_h5_filename,
                                         n_timesamples=self.n_timesamples)
