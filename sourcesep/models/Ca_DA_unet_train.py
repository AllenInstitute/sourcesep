...
def main(expt="ca_da_debug"):
    ...
    datamodule = H5DataModule(
        train_h5_filename=train_h5_filename,
    ...
    ckpt_best_val_loss = ModelCheckpoint(
        monitor="val_loss", mode="min", dirpath=expt_path, filename="best-val-loss_{epoch}", verbose=True
    )
    ckpt_best_val_exp_var = ModelCheckpoint(
        monitor="val_metric_exp_var", mode="max", dirpath=expt_path, filename="best-val-exp-var_{epoch}", verbose=True
    )

    tb_logger = TensorBoardLogger(
        paths["root"] / "results", name="lt_logs", version=expt, prefix="", log_graph=False, default_hp_metric=True
    )
    ...
    model = LitBaseUnet(in_channels=180, S=S, W=W, E=E, Mu_ox=Mu_ox, Mu_dox=Mu_dox, B=B).float()
