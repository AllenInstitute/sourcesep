...
def main(input_file="nmf_single_indicator_test_data", result_file="00_nmf_single_indicator_fit"):
    paths = load_config(dataset_key="all")
    fname = paths["root"] / "sims" / f"{input_file}.pkl"

    assert pathlib.Path(fname).resolve(strict=True), f"File {fname} does not exist"

    with open(fname, "rb") as f:
        dat = pickle.load(f)

    A, X, err, init = nmf_recipe_02(
        dat["Y"].copy(),
        A=dat["A_init_0"].copy(),
        X=dat["X_init_0"].copy(),
        rank=5,
        A_keep=dat["A_keep"].copy(),
        X_keep=None,
        grad_type=None,
        step_size_A=1.0,
        step_size_X=1.0,
    ...
    A, X, err, init = nmf_recipe_02(
        dat["Y"].copy(),
        A=res_fine_tune_0["A"].copy(),
        X=res_fine_tune_0["X"].copy(),
        rank=5,
        A_keep=dat["A_keep"].copy(),
        X_keep=None,
        grad_type=None,
    ...
    A, X, err, init = nmf_recipe_02(
        dat["Y"].copy(),
        A=res_fine_tune_1["A"].copy(),
        X=res_fine_tune_1["X"].copy(),
        rank=5,
        A_keep=dat["A_keep"].copy(),
        X_keep=None,
        grad_type=None,
    ...
    A, X, err, init = nmf_recipe_02(
        dat["Y"].copy(),
        A=res_fine_tune_2["A"].copy(),
        X=res_fine_tune_2["X"].copy(),
        rank=5,
        A_keep=dat["A_keep"].copy(),
        X_keep=None,
        grad_type=None,
    ...
    A, X, err, init = nmf_recipe_02(
        dat["Y"].copy(),
        A=res_fine_tune_3["A"].copy(),
        X=res_fine_tune_3["X"].copy(),
        rank=5,
        A_keep=dat["A_keep"].copy(),
        X_keep=None,
        grad_type=None,
    ...
    A, X, err, init = nmf_recipe_02(
        dat["Y"].copy(),
        A=res_fine_tune_4["A"].copy(),
        X=res_fine_tune_4["X"].copy(),
        rank=5,
        A_keep=dat["A_keep"].copy(),
        X_keep=None,
        grad_type=None,
    ...
    fname = paths["root"] / "results" / f"{result_file}.pkl"
