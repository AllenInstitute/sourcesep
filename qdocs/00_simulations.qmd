...
As in @eq-single-indicator, fluorescence is linearly related to indicator concentration. A ratio of fluorescence measurements at any time $t$ relative to reference time $t_0$ in the absence of any absorption:
...
If the indicator is known to be activity-independent, then minimizing an equation of the form can yield estimates of $\Delta C_{{\textrm{HbO}}}(t)$ and $\Delta C_{{\textrm{HbR}}}(t)$:

$$
\underset{\Delta C_{{\textrm{HbO}}}(t),\Delta C_{{\textrm{HbR}}}(t)}{\tt{argmin}} \sum_{i=1}^{n} [M(t)_i + \Delta C_{{\textrm{HbO}}}(t) A_i + \Delta C_{{\textrm{HbR}}}(t) B_i - \sigma) ]^2
$$
:::
...
1. Control experiment with dead sensor (i.e. activity independent measurement of fluorescence from the sensor)
2. Control experiment with sensor with some form of triggered activity (stimulation/reward etc)
3. Hemodynamics measurement @zhang2022spectral. 
