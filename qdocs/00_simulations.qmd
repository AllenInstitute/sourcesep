...
$$
\begin{align*}
O_{j}(t,\lambda)  &= [ \left( \sum_{i \in \{\textrm{sensors},\textrm{autofluorescence}\}}{a_{i}(t)s_{i}({\lambda})w_{ij}} \right) &\\
& \times e ^ {\left( - \sum_{k \in \{\textrm{oxy},\textrm{deoxy}\}} 
h_{k}(t)\mu_{k}(\lambda) \right)} \times m(t) 
+ b_{j}(\lambda) ] \times n_{j}(t)
\end{align*}
$${#eq-observation}
...
Autofluorescence has a spectrum @autofluorescencepectra, and it varies with time due to metabolic activity. NADH and FAD are molecules that fluoresce in the 400nm-700nm range, see measurements in aqueous solutions in @islam2013ph.

## Open questions

 - Laser noise: Are there regions of the spectrum this can be read out from?
 - Measure $n_j(t)$, fit a Gaussian process to it for simulations? Can data from individual indicators be helpful for this?
 - Time independent sensor + autofluorescence measurement?
 - debug time domain UNet
 - develop frequency domain model
